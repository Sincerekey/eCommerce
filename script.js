...
const messages = document.getElementById('span_message')
...
function validateForm(event){
...
}else if(validateFname() && validatePhone() && validateLname() && validateEmail() && validateZip()){
    let fstname = document.getElementById('fname').value
    let lstname = document.getElementById('lname').value
    let email = document.getElementById('email').value
    messages.innerHTML = `Thanks for submitting ${fstname} ${lstname}. A representitive Will reach out to 
    ${email} within the next buisness day. Thanks and Have a Good Day`
    ...
    setTimeout(function(){messages.style.display = 'none';}, 7000)
...
// JavaScript for my Products Page
document.getElementById("defaultTab").click();


function opentab(event, tabName){
    // Declaring all the variables and Getting all elements
    let tabContent
    let tabLinks
    ...
    // Removing active class 'active' from tablinks
